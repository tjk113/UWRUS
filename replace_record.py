...
def replace_page_text(cur_page_text: str, parsed_records: str, \
    ...
    # sooo the thing to do is maybe iterate over every other element in
    # parsed_record_times, but that wouldn't work if an element didn't
    # have a specifier_text. i guess i could use isinstance actually.
    # check if it's a tuple or a string... yea maybe that.
    ...
    for cur_record, new_record in zip(parsed_cur_records, new_records):
        ...
        if new_record_time == cur_record_time:
            return (None, None) # main script expects page_text to be none if it's meant to skip over a given record
    ...
    for cur_record, cur_parsed_record, new_record in zip(cur_records, parsed_cur_records, new_records):
        ...
        specifier_text = cur_parsed_record[1]
        new_record_time = new_record[0]

        if specifier_text:
            if specifier_text == 'Best Available Video':
            ...
            new_record = f'[{new_record[1]} {new_record[0]}]'
        igt_text = ' (IGT)' if cur_parsed_record[1] == 'IGT' else ''
...
def replace_record_bowser(cur_page_text: str, new_rta_course_record: tuple[str, str] = None, \
    ...
    and returns the new page text and edit summary
    '''
    records_pattern = re.compile(r'rta_record=(?P<rta_course_record>.+) \(Course\)\] \/ '
                                 +'(?P<rta_reds_record>.+) \(with Red Coins\)\]\n'
                                 +'\|ss_record=(?P<ss_course_record>.+) \(Course\)\] \/ '
                                 +'(?P<ss_reds_record>.+) \(with Red Coins\)\]\n'
...
if __name__ == '__main__':
    page_text = ''
    with open('test_page_100c_3.txt', 'r') as file:
        for line in file:
            page_text += line
    ...
    new_text, edit_summary = replace_record(page_text, new_rta_record=new_rta_100c_wr)
